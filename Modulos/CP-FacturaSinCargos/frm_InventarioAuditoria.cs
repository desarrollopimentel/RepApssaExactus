...
{
    public partial class frm_InventarioAuditoria : Form
    {
        ...
        public frm_InventarioAuditoria(string _base, string _user)
        {
            try
            {
                InitializeComponent();
        ...
        private static frm_InventarioAuditoria m_FormDefInstance;
        private static string _base;
        private static string _user;
        //private static string _pass;

        /// Instancia por defecto
        public static frm_InventarioAuditoria DefInstance
        {
            get
            {
                if (m_FormDefInstance == null || m_FormDefInstance.IsDisposed)
                    m_FormDefInstance = new frm_InventarioAuditoria(_base, _user);
        ...
        private void btnActualizar_Click(object sender, EventArgs e)
        {
            ...
            string TipoFecha = this.cboTipoFecha.EditValue.ToString(); //Documento, Auditoria
            string Moneda = this.cboMoneda.EditValue.ToString();  // Dolar, Local

            string tipo = string.Empty;
            string documento = string.Empty;
            tienda = cboTienda.Text;
            ...
            //Documento, Auditoria
            string _tipo_fecha_doc = TipoFecha.Substring(0, 1).ToUpper();  // D , A
            string _moneda = Moneda.Substring(0, 1).ToUpper();             // D , L
        ...
        public void ObtenerInventarios(DateTime fec_corte, string fec_tipo, string moned, string bod)
        {
            using (WaitDialogForm waitDialog = new WaitDialogForm("Procesando Información....", "Espere por favor.."))
            {
                //dtGuias = LogisticaBL.dtListarGuiasDeRemision_BL(fs1, fs2, zon, tie, opc, Global.vUserBaseDatos);
                dtGuias = LogisticaBL.dtListarInventarioAuditoria_BL(fec_corte, fec_tipo, moned, bod, Global.vUserBaseDatos);
                gcGuias.DataSource = dtGuias;
                //ConfiguraGridGuias();
                ConfiguraGrilla(gvGuias);
            }
        }
        ...
        */
        ...
        {
            gvGuias.OptionsView.ColumnAutoWidth = false;
            gvGuias.BestFitColumns();

            Font fnt = new Font(gvGuias.Appearance.Row.Font.Name, 7);
            gvGuias.Appearance.HeaderPanel.Font = fnt;
            gvGuias.Appearance.Row.Font = fnt;
            gvGuias.Appearance.Row.Options.UseFont = true;
            gvGuias.OptionsView.ShowGroupPanel = false;
            gvGuias.OptionsView.ShowIndicator = false;
            gvGuias.OptionsBehavior.Editable = false;
            gvGuias.OptionsSelection.EnableAppearanceFocusedCell = false;
            //// ordenamiento

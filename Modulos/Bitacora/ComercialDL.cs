...
namespace ApssaExactus
{
    public class ComercialDL
    {
        ...
        // @ORIGEN VARCHAR(250) )
        //AS
        public static DataTable dtObtieneBitacoraDL(DateTime _fecha, string _tienda, string _tipo, string _origen, string _usuario, string db)
        {
            using (SqlConnection conn = new SqlConnection(ConexionDC.ConectarBD(db)))
            {
                string sqlCommand = "PIMENTEL.SP_APSSA_BITACORA_CONSUMER";
                ...
                using (SqlCommand cmd = new SqlCommand(sqlCommand, new SqlConnection(connectionString)))
                {
                    ...
                    cmd.Parameters.AddWithValue("@FECHA_REP", _fecha);

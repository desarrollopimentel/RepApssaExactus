...
namespace ApssaExactus
{
    ...
    public class LogisticaDL
    {
        ...
        //ALTER PROCEDURE[PIMENTEL].[SP_APSSA_AUDITORIA_INVENTARIO]
        //(@FECHA_CORTE DATETIME,
        // @TIPO_FECHA VARCHAR(1),	--D-documento, A-auditoria
        // @MONEDA VARCHAR(1), 		--L-local, D-dolar
        // @BODEGA VARCHAR(MAX))
        public static DataTable dtListarInventarioAuditoria_DL(DateTime _fecha_corte, string _tipo_fecha, string _moneda, string _bodega, string db)
        {
            using (SqlConnection conn = new SqlConnection(ConexionDC.ConectarBD(db)))
            {
                string sqlCommand = "PIMENTEL.SP_APSSA_AUDITORIA_INVENTARIO";
        ...
        //ALTER PROCEDURE[PIMENTEL].[SP_APSSA_GRE]
        //(@FECHA_INI DATETIME,
        //@FECHA_FIN DATETIME,
        //@ZONA VARCHAR(MAX),
        //@TIENDA VARCHAR(MAX),
        //@OPCION VARCHAR(10))
        public static DataTable dtListarGuiasDeRemision_DL(DateTime _fini, DateTime _fina, string _zon, string _tie, string _opc, string db)
        {
            using (SqlConnection conn = new SqlConnection(ConexionDC.ConectarBD(db)))
            {
                string sqlCommand = "PIMENTEL.SP_APSSA_GRE";
                string connectionString = ConexionDC.ConectarBD(db);
                DataSet ds = new DataSet();
                using (SqlCommand cmd = new SqlCommand(sqlCommand, new SqlConnection(connectionString)))
                {
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.AddWithValue("@FECHA_INI", _fini);
                    cmd.Parameters.AddWithValue("@FECHA_FIN", _fina);
                    cmd.Parameters.AddWithValue("@ZONA", _zon);
                    cmd.Parameters.AddWithValue("@TIENDA", _tie);
                    cmd.Parameters.AddWithValue("@OPCION", _opc);
                    cmd.CommandTimeout = 0;
                    cmd.Connection.Open();
                    DataTable table = new DataTable();
                    table.Load(cmd.ExecuteReader());
        ...
        //ALTER PROCEDURE[PIMENTEL].[SP_APSSA_RESERVA_BITACORA]
        //(@FECHA_INI DATETIME,
        //@FECHA_FIN DATETIME,
        //@OPCION VARCHAR(10) )  -- PROCESO, DOCUMENTO
        public static DataTable dtListarBitacoraDesreserva_DL(DateTime _fecha_ini, DateTime _fecha_fin, string _opcion, string db)
        {
            string strSql = "PIMENTEL.SP_APSSA_RESERVA_BITACORA";
            List<SqlParameter> arParams = new List<SqlParameter>();

            arParams.Add(new SqlParameter("@FECHA_INI", _fecha_ini));
            arParams.Add(new SqlParameter("@FECHA_FIN", _fecha_fin));
            arParams.Add(new SqlParameter("@OPCION", _opcion));

            return SqlHelper.ExecuteDataset(ConexionDC.ConectarBD(db), CommandType.StoredProcedure, strSql, arParams.ToArray()).Tables[0];
        }


        //ALTER PROCEDURE[PIMENTEL].[SP_APSSA_RESERVA_ELIMINAR]
        //(@VAR_TIPODOC VARCHAR(3),				-- PED FAC BOL
        // @VAR_DOCUMENTO VARCHAR(50),			-- P005-00041594
        // @VAR_BODEGA VARCHAR(4),				-- 0003
        // @VAR_ARTICULO VARCHAR(20),			-- PLO3003
        // @VAR_APLICACION VARCHAR(50),		    -- PED#P005-00041594.3
        // @VAR_CANTIDAD  FLOAT,					-- 1 
        // @VAR_USUARIO VARCHAR(25) )				--
        public static bool EliminarReservaArticulo_DL(string _tipodoc, string _documento, string _bodega, string _articulo, string _aplicacion, Decimal _cantidad, string _usuario, string db)
        {
            int NumReg = 0;
            string strSql = "PIMENTEL.SP_APSSA_RESERVA_ELIMINAR";

            List<SqlParameter> arParam = new List<SqlParameter>();
            arParam.Add(new SqlParameter("@VAR_TIPODOC", _tipodoc));
            arParam.Add(new SqlParameter("@VAR_DOCUMENTO", _documento));
            arParam.Add(new SqlParameter("@VAR_BODEGA", _bodega));
            arParam.Add(new SqlParameter("@VAR_ARTICULO", _articulo));
            arParam.Add(new SqlParameter("@VAR_APLICACION", _aplicacion));
            arParam.Add(new SqlParameter("@VAR_CANTIDAD", _cantidad));
            arParam.Add(new SqlParameter("@VAR_USUARIO", _usuario));

            NumReg = SqlHelper.ExecuteNonQuery(ConexionDC.ConectarBD(db), CommandType.StoredProcedure, strSql, arParam.ToArray());
            //si todo fue OK retorna -1
            return NumReg < 0;
        }
        ...
        public static DataTable dtListarArticulosReservados_DL(DateTime fini, DateTime fina, string zon, string tip, string docu, string db)
        {
            string strSql = "PIMENTEL.SP_APSSA_ARTICULOS_RESERVADOS_V3";
            List<SqlParameter> arParams = new List<SqlParameter>();
